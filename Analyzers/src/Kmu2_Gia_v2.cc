...
void Kmu2::InitHist(){
    ...
    //BookHisto(new TH1F("MUV3_MUV2_noMUV1tdiff" , " MUV3_{time} - MUV2_{time} ; MUV3_{time} - MUV2_{time} [ns]", 100, -50, 50.));
    //BookHisto(new TH1F("MUV3_MUV1_nuMUV2tdiff" , " MUV3_{time} - MUV1_{time} ; MUV3_{time} - MUV1_{time} [ns]", 100, -50, 50.));
    //BookHisto(new TH1F("MUV2_MUV1_nuMUV3tdiff" , " MUV2_{time} - MUV1_{time} ; MUV2_{time} - MUV1_{time} [ns]", 100, -50, 50.));
    ...
    //BookHisto(new TH2I("MUV13_2Hits_M1", "Events that have no reconstructed cluster, but have hits in at least one of the strips where the track cross (MUV1+3 events)", 44, 0, 44, 44, 0, 44));
    //BookHisto(new TH2I("MUV13_1Hit_M1" , "Events that have no reconstructed cluster, but have hits in at least one of the strips where the track cross (MUV1+3 events)", 44, 0, 44, 44, 0, 44));
...
void Kmu2::Process(int iEvent){
    ...
    double LKrOffset   = +121.2; //Old 112.3 Use GetClusterTime new 115 Gia 121.2
    double CedarOffset = 6.23; //old 0 new 0 Gia 6.23
    double MUV1Offset  = 6.08;//new -5 old -8.5 Gia 6.08
    double MUV2Offset  = 5.59;//new -5 old -20.4
    double MUV3Offset  = -2.75;//old -11.35

    //Cuts used on the timedifferences for the Kmu2 selection
    double LKrOffsetCut  = 10;
    double RICHOffsetCut = 10;
    double MUV3OffsetCut = 5 ;
    double MUV1OffsetCut = 20;
    double MUV2OffsetCut = 20;
    double CedarOffsetCut= 2 ;
    ...
    if(MUV2TrackClusterIndex > -1){
        ...
        if( fabs(CD_MUV2Cluster->GetPosition().X() - MUV2_extrap.X()) > 300. ||
            fabs(CD_MUV2Cluster->GetPosition().Y() - MUV2_extrap.Y()) > 300.){return;}
    }
    //Before
    //cout << "Before --" << endl;
    ...
    if(MUV3TrackClusterIndex > -1 &&
       ...
       MUV1TrackClusterIndex < 0 ){
        ...
        for(int iMUV1Hit=0;  iMUV1Hit <  MUV1Event->GetNHits(); iMUV1Hit++){
            ...
            //cout << "MUV1ChID == " << MUV1Hit->GetChannelID()%50 - 1 << endl;
